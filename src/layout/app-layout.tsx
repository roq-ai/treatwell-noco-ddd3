...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['salon-owner', 'stylist', 'receptionist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SalonInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/salons` : null),
    () => getSalons({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Salon',
      path: isTenantUser ? `/salons/view/${data?.[0]?.id}` : '/salons',
      entity: 'salon',
    ...
    { name: 'Customer', path: '/customers', entity: 'customer', service: AccessServiceEnum.PROJECT, icon: FiPhoneCall },
