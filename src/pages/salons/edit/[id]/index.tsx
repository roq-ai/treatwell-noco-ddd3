...
import { getSalonById, updateSalonById } from 'apiSdk/salons';
...
function SalonEditPage() {
  ...
  const handleSubmit = async (values: SalonInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSalonById(id, values);
      mutate(updated);
      resetForm();
      router.push('/salons');
    } catch (error) {
  ...
  const formik = useFormik<SalonInterface>({
    initialValues: data,
